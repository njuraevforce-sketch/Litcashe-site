...
(function(){
  ...
  // ---------------- AUTH ----------------
  async function registerUser({email, password, phone, ref}){
...
async function logout(){
  ...
  document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if(regForm){
      regForm.addEventListener('submit', async (e)=>{
        e.preventDefault();
        try{
          await registerUser({ email: $('#regEmail')?.value.trim(), phone: $('#regPhone')?.value.trim(), password: $('#regPass')?.value, ref: $('#refId')?.value.trim() });
          alert('Регистрация успешна! Подтвердите email, затем войдите.'); location.href='login_single.html';
  ...
  async function refreshDashboard(){
    ...
    const pText = d.progress || ''; const pEl = $('#progressText'); if(pEl) pEl.textContent = pText;
    ...
    try{
      ...
      if(u && refLink){
        ...
        const code = rc?.data?.ref_code || ''; refLink.value = `${location.origin}/register_single.html?ref=${code}`;
      }
    }catch(e){}
  }
