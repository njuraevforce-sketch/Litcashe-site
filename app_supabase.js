...
;(function () {
  ...
  LC.refreshLevelInfo = async function() {
    try {
      ...
      try {
        const levelCards = document.querySelectorAll('.level-card, .level-card-carousel');
        if (levelCards.length) {
          levelCards.forEach(card => {
            const cardLevel = card.getAttribute('data-level');
            const statusElement = card.querySelector('.level-status, .level-vip-badge');
            ...
            if (statusElement) {
              ...
              // Если нет элемента статуса, просто добавляем класс active
              if (cardLevel === info.level_name?.toLowerCase().replace(' ', '')) {
                card.classList.add('active');
              }
            }
  ...
  LC.loadReferralEarnings = async function() {
    try {
      ...
      set('#refTotalChart', fmtMoney(total));
      set('#gen1Chart', fmtMoney(pickNum(gen1.total_cents)/100));
      set('#gen2Chart', fmtMoney(pickNum(gen2.total_cents)/100));
      set('#gen3Chart', fmtMoney(pickNum(gen3.total_cents)/100));
  ...
  LC.requestWithdrawal = async function(amountCents, method = 'TRC20', address = '') {
    try {
      ...
      const result = typeof data === 'object' ? data : (data ? JSON.parse(data) : {});
      ...
      alert('✅ ' + (result.message || 'Заявка на вывод создана успешно'));
